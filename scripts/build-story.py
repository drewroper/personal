...
Layout rules shared by every variant:
  ...
  * The background motion is chosen per cover, for the most interesting dither (Drew). Busy covers
    (MPP, Wondrous Bughouse, Weezer) need nothing: the default slow drift around the centre. A flat or
    low-contrast cover gets its own `story` move in albums.json, e.g. TEB: aim at the 3b logo, lift the
    levels, one straight steady pan with no easing. Don't copy one cover's move onto another.
...
def _ground(art, theta, riff):
    ...
    if "detail" in riff:
        ...
        if ONE_SHOT and "focus" in look:
            # A per-record move, tuned to that cover (TEB: start centred, pan straight and steady so the
            # 3b logo comes in from the left and ends cropped by the top edge). Linear, no easing, and
            # the crop stays inside the art, a little in from its worn edge.
            m = int(side * .03)
            ...
            amp = min(int(90 * z / 3), (side - W) // 2, (side - H) // 2)
            cx = (side - W) // 2 + int(math.cos(t) * amp); cy = (side - H) // 2 + int(math.sin(t) * amp)
