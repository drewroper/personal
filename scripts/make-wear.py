...
def make(slug, year=None):
    ...
    life = float(np.clip(.22 + age / 40 * 1.15 + U(-.12, .12), .2, 1.45))
    ...
    # Ring wear: the disc's edge abrading the print. Feathered band, heaviest where the disc rested.
    if P(.78):
        cx, cy = S * U(.47, .53), S * U(.47, .53); R = S * U(.43, .47)
        r = np.hypot(xx - cx, yy - cy); ang = np.arctan2(yy - cy, xx - cx)
        w = S * U(.012, .03)
        band = np.exp(-((r - R) / w) ** 2) + .35 * np.exp(-((r - R) / (w * 2.6)) ** 2)
        rest = U(0, 2 * math.pi); lobe = .35 + .65 * (.5 + .5 * np.cos(ang - rest)) ** U(1, 3)
        ring = band * lobe * (.5 + noise(30) * .9) * (noise(4, 2) > U(.25, .45))
        light += blur(ring, .6) * U(.8, 1.3)
        dark += np.exp(-((r - R * 1.035) / (w * 1.4)) ** 2) * lobe * (.3 + noise(40) * .6) * U(.15, .4)
        if P(.3):                                                        # a second, offset ring from an inner sleeve
            R2 = R * U(.93, .99); cx2, cy2 = cx + S * U(-.03, .03), cy + S * U(-.03, .03)
            r2 = np.hypot(xx - cx2, yy - cy2)
            light += blur(np.exp(-((r2 - R2) / (w * .8)) ** 2) * (.3 + noise(26) * .7) * (noise(4, 2) > .3), .8) * U(.3, .6)
        if P(.5):                                                        # faint spindle/label ring
            light += np.exp(-((r - S * U(.15, .19)) / (S * .01)) ** 2) * noise(24) * U(.15, .35)
    ...
    a_l = light; a_d = dark
    rgb_l = np.array([248, 246, 240], np.float32); rgb_d = np.array([22, 20, 18], np.float32)
    A = a_d + a_l * (1 - a_d)
